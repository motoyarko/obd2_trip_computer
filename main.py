...
def get_values():
    ...
    while STOP_GET:
        ...
        if response.value is not None:
            CONTROL_MODULE_VOLTAGE = str_int(str(response.value))
            #CONTROL_MODULE_VOLTAGE = str_int(str("pussy"))
...
def print_screen(screen_number):
    if screen_number is 0:
        ...
        if odometr > 0.1: # отображать расход на 100 км только после 100 метров пробега
            lp100_to_print = "{:.1f}".format(LPH)
        else:
            lp100_to_print = "-.-"
        print_text_topright(140, 85, lp100_to_print, 40, fill=(2, 135, 178))
        print_text_topleft(150, 85, "L/h", 40, fill=(2, 135, 178))

        # Print trip L/100
        print_text_topright(140, 125, "{:.1f}".format(LP100), 40, fill=(2, 135, 178))
        ...
        print_text_topright(490, 290, CONTROL_MODULE_VOLTAGE, 40, fill=(2, 135, 178))
...
while not done:
    ...
    FuelFlowGramsPerSecond = (MAF / AirFuelRatio) * ls_term_val # // Получаем расход грамм бензина в секунду в соотношении 14,7 воздуха/к 1 литра бензина, корректировка ls_term_val
